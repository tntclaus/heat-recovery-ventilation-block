...
module place_fans4square_vent204x60(w) {
    translate([w, 0, 0])
        children();
...
* Заглушка на канал
*/
module fan_holder_square_vent204x60_assembly(fan_type) {
    assembly("fan_holder_square_vent204x60") {
        translate_z(- 15.01 - 14.4 / 2)
        fan4square_vent204x60_cap(fan_type);
...
module fan4square_vent204x60_cap(fan_type) {
    stl(str("ABS_fan4square_vent204x60_cap_fan", fan_width(fan_type), "x", fan_depth(fan_type)));
    module fan_base() {
        difference() {
            square_vent_channel_cube(square_vent208x64, 2);

            place_fans4square_vent204x60(65){
                fan_holes(fan_type);
            }
        }
    }
    ...
    translate_z(6)
    fan_base();
    square_vent_channel_model(square_vent208x64, 10);
}

module fan4square_vent204x60_base(fan_type) {
    stl(str("ABS_fan4square_vent204x60_base_fan", fan_width(fan_type), "x", fan_depth(fan_type)));
...
module fan_outblower_assembly() {
    ...
    assembly("fan_outblower") {
        translate_z(30 / 2) {
            ...
            translate_z(- 5.01)
            fan4square_vent204x60_base(fan60x20);
        }

        fan_holder_square_vent204x60_assembly(fan60x20);
    }
}


module ABS_fan4square_vent204x60_cap_fan60x20_stl() {
    fan4square_vent204x60_cap(fan60x20);
}
...
module ABS_fan4square_vent204x60_base_fan60x20_stl() {
    fan4square_vent204x60_base(fan60x20);
}
